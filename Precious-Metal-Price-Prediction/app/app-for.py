...
def show_section(asset):
    ...
    try:
        ...
        delta_display = f"{'▲' if pred_change > 0 else '▼'} {abs(pred_change):.2f}"
        ...
        st.caption(delta_display)
        
    except:
        col1.metric("🎯 Predicted Next Day", "N/A")

    delta_display2 = f"{'▲' if change > 0 else '▼'} {abs(change):.2f}"
    col2.metric("📅 Selected Day",f"₹ {today_price:.2f}",change)
    st.caption(delta_display2)
